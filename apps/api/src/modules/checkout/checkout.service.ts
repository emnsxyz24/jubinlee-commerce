...
export class CheckoutService {
  ...
  async processCheckout(dto: CheckoutDto, customerId?: string) {
    ...
    while (!isUnique) {
      ...
      orderNumber = `ORD-${todayStr}-${randSuffix}`;
    ...
    const orderItemsData = cart.cartItems.map((item) => {
      ...
      const vConfig = (item.variant.product.attributes as any)?.variantConfig;
      let customOptText: string | null = null;
      if (vConfig?.customVariantEnabled) {
        const cGroup = vConfig.customVariant || vConfig.customVariants?.[0];
        if (cGroup?.name && cGroup?.options?.[0]?.name) {
          customOptText = `${cGroup.name}: ${cGroup.options[0].name}`;
        }
      }
    ...
    let messageText = `Halo, saya ingin memesan dari Jubi & Lee:\n\n`;
  ...
  async getOrderConfirmation(orderNumber: string) {
    ...
    const items = order.orderItems.map((item) => {
      ...
      if (vColor) {
        ...
        if (colorObj?.imageUrl) imageUrl = colorObj.imageUrl;
      }
      if (!imageUrl) {
        imageUrl = item.variant?.product?.productImages?.[0]?.url || null;
      }
