...
async function main() {
  ...
  const sampleProducts = [
    ...
    },
    {
      ...
      images: ['https://images.unsplash.com/photo-1548883354-7622d03aca27?q=80&w=800&auto=format&fit=crop'],
      vars: [{ sku: 'OW-USK-OS-NVY', size: undefined, color: 'Navy', hex: '#000080', stock: 15 }],
    },
...

    {
      ...
      vars: [
        { sku: 'AC-LTB-OS-BRN', size: undefined, color: 'Brown', hex: '#8B4513', stock: 18 },
        { sku: 'AC-LTB-OS-BLK', size: undefined, color: 'Black', hex: '#000000', stock: 25 },
      ],
    },
    {
      ...
      vars: [{ sku: 'AC-SCP-OS-TAN', size: undefined, color: 'Tan', hex: '#D2B48C', stock: 6 }],
    },
    {
      ...
      vars: [{ sku: 'AC-CCP-OS-GLD', size: undefined, color: 'Gold', hex: '#FFD700', stock: 15 }],
    },
    {
      ...
      vars: [{ sku: 'AC-WSH-OS-NAT', size: undefined, color: 'Natural', hex: '#EEDC82', stock: 4 }], // Low stock
    },
    {
    ...
    },
    {
      ...
      vars: [{ sku: 'AC-OSS-OS-TRT', size: undefined, color: 'Tortoise', hex: '#800000', stock: 8 }],
    },
    {
      ...
      vars: [{ sku: 'AC-SCS-OS-GRY', size: undefined, color: 'Camel', hex: '#C19A6B', stock: 3 }], // Low stock
    },
    {
      ...
      vars: [{ sku: 'AC-CWD-OS-OLV', size: undefined, color: 'Olive', hex: '#556B2F', stock: 7 }],
    },
    {
      ...
      vars: [{ sku: 'AC-MGH-OS-GLD', size: undefined, color: 'Gold', hex: '#FFD700', stock: 12 }],
    },
    {
      ...
      vars: [{ sku: 'AC-LCW-OS-BLK', size: undefined, color: 'Black', hex: '#000000', stock: 0 }], // Out of stock
    },
    {
      ...
      images: ['https://images.unsplash.com/photo-1601924994987-69e26d50dc26?q=80&w=800&auto=format&fit=crop'],
      vars: [{ sku: 'AC-SPH-OS-PRT', size: undefined, color: 'Terracotta Print', hex: '#E2725B', stock: 10 }],
    },
    {
  ...
  // ----------------------------------------------------
  // DUMMY CUSTOMERS & ADDRESSES
  // ----------------------------------------------------
  console.log('Seeding dummy customers & addresses...');
  const customerPasswordHash = await bcrypt.hash('CustomerPass123!', 10);
  const customersData = [
    {
      name: 'Dinda Kirana',
      email: 'dinda.kirana@example.com',
      phone: '081234567891',
      isGuest: false,
      address: {
        label: 'Rumah',
        recipientName: 'Dinda Kirana',
        phone: '081234567891',
        line1: 'Jl. Senopati No. 12',
    ...
    },
    {
      ...
      address: {
        label: 'Apartemen',
        recipientName: 'Raka Pratama',
        phone: '081987654321',
        line1: 'Jl. Dago Asri No. 8',
        line2: 'Unit 12B',
        city: 'Bandung',
        province: 'Jawa Barat',
        postalCode: '40135',
  ...
  // ----------------------------------------------------
  // DUMMY ORDERS & SALES HISTORY (LAST 30 DAYS)
  // ----------------------------------------------------
  console.log('Seeding realistic dummy orders and sales trends...');
  ...
  if (allVariants.length > 0) {
    ...
    const mockOrdersConfig = [
      { daysAgo: 28, status: 'delivered' as const, customerIdx: 0, variantOffsets: [0, 1] },
      { daysAgo: 25, status: 'delivered' as const, customerIdx: 1, variantOffsets: [2] },
      { daysAgo: 22, status: 'delivered' as const, customerIdx: 2, variantOffsets: [3, 4] },
      { daysAgo: 19, status: 'delivered' as const, customerIdx: 3, variantOffsets: [5] },
      { daysAgo: 17, status: 'delivered' as const, customerIdx: 4, variantOffsets: [6, 7] },
      { daysAgo: 14, status: 'delivered' as const, customerIdx: 0, variantOffsets: [8] },
      { daysAgo: 12, status: 'delivered' as const, customerIdx: 1, variantOffsets: [9, 10] },
      { daysAgo: 10, status: 'delivered' as const, customerIdx: 2, variantOffsets: [11] },
      { daysAgo: 8, status: 'shipped' as const, customerIdx: 3, variantOffsets: [12, 13] },
      { daysAgo: 6, status: 'shipped' as const, customerIdx: 4, variantOffsets: [14] },
      { daysAgo: 5, status: 'processing' as const, customerIdx: 0, variantOffsets: [15, 16] },
      { daysAgo: 4, status: 'confirmed' as const, customerIdx: 1, variantOffsets: [17] },
      { daysAgo: 3, status: 'confirmed' as const, customerIdx: 2, variantOffsets: [18, 19] },
      { daysAgo: 2, status: 'contacted' as const, customerIdx: 3, variantOffsets: [20] },
      { daysAgo: 1, status: 'pending_whatsapp' as const, customerIdx: 4, variantOffsets: [21, 22] },
      { daysAgo: 0, status: 'pending_whatsapp' as const, customerIdx: 0, variantOffsets: [23] },
      { daysAgo: 15, status: 'cancelled' as const, customerIdx: 1, variantOffsets: [24] },
    ];

    for (let i = 0; i < mockOrdersConfig.length; i++) {
      const cfg = mockOrdersConfig[i];
      const targetCustomer = createdCustomers[cfg.customerIdx % createdCustomers.length];
      const orderDate = new Date(Date.now() - cfg.daysAgo * 24 * 60 * 60 * 1000);
      ...
      if (!existingOrder) {
        ...
        for (const offset of cfg.variantOffsets) {
          const v = allVariants[offset % allVariants.length];
          ...
          orderItemsToCreate.push({
            ...
            variantSnapshot: [v.size ? `Size: ${v.size}` : null, v.color ? `Color: ${v.color}` : null].filter(Boolean).join(' / ') || 'Standard',
            skuSnapshot: v.sku,
            unitPrice,
            quantity: qty,
            lineTotal,
        ...
        const addressSnapshot = {
          recipientName: targetCustomer.name,
        ...
        const waMsg = `*PESANAN BARU #${orderNumber}*\n\nNama: ${targetCustomer.name}\nWhatsApp: ${targetCustomer.phone}\nAlamat: Jl. Senopati No. 12, Jakarta Selatan\nTotal: Rp${subtotal.toLocaleString('id-ID')}\n\nMohon konfirmasi pembayaran & pengiriman. Terima kasih!`;
        ...
        await prisma.orderStatusHistory.create({
          data: {
            ...
            toStatus: cfg.status,
            changedBy: adminId,
            note: `Initial status ${cfg.status} generated in dataset`,
            createdAt: orderDate,
          },
        });
      }
  ...
  const contactMessagesData = [
    {
      ...
      email: 'dinda.kirana@example.com',
      phone: '081234567891',
      subject: 'Restock Oversized Linen Shirt',
      message: 'Halo Admin Jubi & Lee, apakah Oversized Linen Shirt warna Terracotta ukuran M akan restock minggu depan?',
    ...
    },
    {
      ...
      phone: '081399887766',
      subject: 'Pertanyaan Pengiriman ke Luar Pulau',
      message: 'Mau tanya estimasi ongkir dan durasi pengiriman menggunakan ekspedisi kilat ke Balikpapan berapa hari ya?',
    ...
    },
    {
      ...
      phone: '085711223344',
      subject: 'Permintaan Katalog Grosir / Reseller',
      message: 'Halo Jubi & Lee Studio, kami butik fashion di Semarang tertarik untuk menjalin kerjasama reseller. Apakah ada program kemitraan?',
    ...
    },
    {
      ...
      phone: '081298765432',
      subject: 'Konfirmasi Pengembalian Ukuran',
      message: 'Selamat sore, blazer yang saya pesan ukurannya agak kekecilan di pundak, apakah bisa ditukar ke ukuran L?',
      status: 'unread' as const,
    },
  ];

  for (const m of contactMessagesData) {
  ...
  // DUMMY ACTIVITY LOGS
  // ----------------------------------------------------
  console.log('Seeding dummy system activity logs...');
  const adminUser = await prisma.adminUser.findFirst();
  if (adminUser) {
    const activityLogsData = [
      { action: 'auth.login', entityType: 'admin_user', entityId: adminUser.id, afterData: { email: adminUser.email, ip: '127.0.0.1' } },
      { action: 'catalog.publish', entityType: 'product', afterData: { status: 'published', note: 'Bulk publish Summer Drop 2026' } },
      { action: 'order.status_update', entityType: 'order', afterData: { toStatus: 'confirmed', note: 'Pembayaran DP 100% terverifikasi via WA' } },
      { action: 'inventory.adjust', entityType: 'product_variant', afterData: { delta: 15, reason: 'Restock batch penerimaan garmen' } },
    ];
  ...
  console.log(`Successfully seeded comprehensive fashion store ecosystem!`);
}
