<template>
  <main class="w-full flex-1 pt-24 sm:pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
    ...
    >
      <div 
        class="bg-[#FAF6F1] border border-[#E4D8CC] rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative max-h-[90vh] overflow-y-auto cursor-default"
        @click.stop
      >
...
const handleSaveAddress = async () => {
  try {
    ...
    if (editingId.value) {
      ...
      toast.success('Address Updated', 'Delivery address has been updated.');
    } else {
    ...
    toast.error('Failed to save address', e?.data?.message || e?.message || 'Failed to save address');
  } finally {
...
const handleDelete = async (id: string) => {
  ...
  try {
    ...
    toast.info('Address Removed', 'The address was removed from your account.');
    await loadAddresses();
  } catch (e: any) {
    toast.error('Failed to delete address', e?.data?.message || e?.message || 'Failed to delete address');
  }
};
</script>
