  ...
  <main class="w-full flex-1 pt-24 sm:pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
      ...
      <div class="lg:col-span-5 bg-[#FAF6F1] p-6 sm:p-8 rounded-3xl border border-[#E4D8CC] shadow-md space-y-6 lg:sticky lg:top-28">
        ...
        <form @submit.prevent="handleCheckout" class="space-y-4 pt-4 border-t border-[#E4D8CC]" novalidate>
          ...
          >
            <span>{{ submitting ? 'Processing Order...' : 'Checkout' }}</span>
          </button>
        </form>
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div 
          v-if="itemToRemove" 
          class="fixed inset-0 z-[20000] bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 cursor-default"
          role="dialog"
          aria-modal="true"
          @click="itemToRemove = null"
        >
          <div class="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-100 text-center space-y-4 cursor-default" @click.stop>
...
const confirmRemoveItem = async () => {
  if (itemToRemove.value) {
    ...
    toast.info('Item Removed', `"${name}" has been removed from your bag.`);
  }
};
...
const handleCheckout = async () => {
  ...
  try {
    ...
    if (res?.orderNumber) {
      await router.push(`/order/confirmation/${res.orderNumber}`);
    }
  } catch (e: any) {
