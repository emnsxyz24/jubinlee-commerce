<template>
  <main class="bg-[#F4ECE5] min-h-screen text-[#1A170F] pt-24 sm:pt-28 pb-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div v-else class="space-y-6 sm:space-y-8">
        ...
        <div class="bg-[#FAF6F1] border border-[#E4D8CC] rounded-2xl p-4 sm:p-6 text-[#1A170F] text-xs leading-relaxed flex items-start gap-3.5 shadow-xs">
          ...
          <div class="space-y-1">
            <span class="font-extrabold text-[#1A170F] block">Next step: Chat with our store specialist on WhatsApp</span>
            <p class="text-[#1A170F]/80">
              Click the <strong class="text-[#E04F26]">"Open WhatsApp Chat"</strong> button above to start your order conversation, or click <strong class="text-[#1A170F]">"Copy Order Text"</strong> and paste it directly into your WhatsApp message to our store number <strong class="text-[#1A170F]">+62 812 3456 7890</strong>.
            </p>
          </div>
        </div>
        ...
        <div class="bg-[#FAF6F1] p-6 sm:p-10 rounded-3xl border border-[#E4D8CC] shadow-sm space-y-6 sm:space-y-8">
          ...
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-[#E4D8CC] gap-3">
            ...
            <span 
              ...
              class="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-bold border uppercase tracking-wider shadow-xs"
            >
              ...
              >
                <div class="flex items-center gap-3 min-w-0">
                  <div class="w-12 h-14 rounded-xl overflow-hidden bg-[#F4ECE5] border border-[#E4D8CC] shrink-0">
                    <img 
                      :src="item.imageUrl || item.productImage || 'https://via.placeholder.com/100x120'" 
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-[#1A170F]/80 text-[11px] mt-0.5">
                      Qty: {{ item.quantity }} × Rp{{ formatPrice(item.unitPrice || item.unitPriceSnapshot) }}
                    </p>
                  </div>
                </div>
          ...
          <div class="bg-[#F4ECE5] p-4 sm:p-5 rounded-2xl text-xs text-[#1A170F]/80 leading-relaxed border border-[#E4D8CC] font-light space-y-1">
            <p class="font-bold text-[#1A170F] text-xs">💬 How WhatsApp Checkout Works:</p>
            <p>
              Your order has been recorded. Click the <strong class="text-[#E04F26]">"Open WhatsApp Chat"</strong> button to send your pre-formatted order summary to our store desk. Our team will verify item availability, provide shipping options, and share payment details directly on WhatsApp.
            </p>
          </div>
