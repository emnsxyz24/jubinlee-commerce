  ...
  >
    <button
      ...
      :class="[
        ...
        isOpen ? 'border-[#E04F26] ring-1 ring-[#E04F26]/30' : 'border-[#E4D8CC]',
        disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-[#E04F26] cursor-pointer'
      ]"
      class="bg-[#FAF6F1] border text-[#1A170F] font-bold flex items-center gap-1.5 transition shadow-2xs select-none max-w-full"
      :title="selectedLabel"
    ...
    >
      ...
      <svg 
        ...
        class="w-3 h-3 transition-transform duration-200 shrink-0 ml-auto" 
        fill="none" 
        stroke="currentColor" 
        stroke-width="2.5" 
        viewBox="0 0 24 24"
    ...
    >
      <div
        ...
        :class="[
          ...
          fullWidth ? 'min-w-full' : 'min-w-[180px]',
          compact ? 'p-1 rounded-xl' : 'p-1.5 rounded-2xl'
        ]"
        class="absolute top-full mt-1.5 bg-[#FAF6F1] border border-[#E4D8CC] shadow-xl z-50 max-h-60 overflow-y-auto"
      >
        <button
          ...
          :class="[
            ...
            isSelected(opt.value)
              ? 'bg-[#1A170F] text-[#FAF6F1] font-extrabold'
              : opt.disabled
                ? 'text-[#1A170F]/30 cursor-not-allowed'
                : 'text-[#1A170F]/80 hover:bg-[#E4D8CC]/50 hover:text-[#1A170F] font-semibold cursor-pointer'
          ]"
