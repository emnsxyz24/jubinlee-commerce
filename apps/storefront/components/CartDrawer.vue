<template>
  <div v-if="cartStore.isDrawerOpen" class="relative z-[10000]">
    
    <div 
      @click="cartStore.toggleDrawer()" 
      class="fixed inset-0 bg-slate-950/60 backdrop-blur-xs transition-opacity cursor-default"
      aria-hidden="true"
    ></div>

    <div class="fixed inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
      <div class="w-screen max-w-md bg-[#F4ECE5] text-[#1A170F] shadow-2xl flex flex-col isolate contain-content border-l border-[#E4D8CC] relative">
        ...
        <div class="flex-1 min-h-0 relative overflow-hidden flex flex-col">
          <div 
            ...
            class="flex-1 overflow-y-auto p-6 space-y-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden overscroll-contain"
          >
          ...
          <ModalScrollbar :target="scrollContainerRef" />
        </div>
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div 
          v-if="itemToRemove" 
          class="fixed inset-0 z-[20000] bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 cursor-default"
          role="dialog"
          aria-modal="true"
          @click="itemToRemove = null"
        >
          <div class="bg-[#FAF6F1] rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-[#E4D8CC] text-center space-y-4 text-[#1A170F] cursor-default" @click.stop>
...
const changeItemVariant = async (itemId: string, newVariantId: string) => {
  try {
    ...
    toast.error('Failed to update variant', e?.data?.message || e?.message || 'Failed to update variant');
  } finally {
...
const confirmRemoveItem = async () => {
  if (itemToRemove.value) {
    ...
    const name = itemToRemove.value.name;
    itemToRemove.value = null;
    await cartStore.removeItem(id);
    toast.info('Item Removed', `"${name}" has been removed from your bag.`, 'bottom-left');
  }
};
...
const getItemImage = (item: any) => {
  ...
  if (vColor) {
    const vConfig = item.variant?.product?.attributes?.variantConfig;
    const colorObj = vConfig?.colors?.find((c: any) => c.name?.toLowerCase() === vColor.toLowerCase());
    if (colorObj?.imageUrl) return colorObj.imageUrl;
  }
  return item.variant?.product?.productImages?.[0]?.url || 'https://via.placeholder.com/150';
};
